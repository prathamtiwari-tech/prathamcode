...
public class Cache {		
	...
	private int next = 0;
	private static final Comparator<Bookmark> ratingComparator=new Comparator<Bookmark>() {
		
		{
			System.out.print("Intantiated");
		}
		
		//Member inner class can have static and final fields but it must be initialized at the time of decalaration only.
...
//		});
		...
		Arrays.sort(recommendedItems.items,ratingComparator);
		
		
		Cache cache=new Cache(6);
		
		//Cache.MyCacheIterator i=cache.new MyCacheIterator();
